...
public class ImageSVM {
	...
	public double getAccuracy(Picture picture){
		double[] doubImg;
		if(tinyImage){
			doubImg = picture.getTinyImageDouble();
		} else{
			doubImg = picture.getArrayHistogram();
		}
		
		svm_node[] nodes = new svm_node[doubImg.length];
	    for (int i = 0; i < doubImg.length; i++)
	    {
	        svm_node node = new svm_node();
	        node.index = i;
	        node.value = doubImg[i];
